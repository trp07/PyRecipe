    ...
    def validate_email(self, email):
        ...
        if user:
            raise ValidationError(
                "Email address in use.  Please use a different email address."
            )
...
class RecipeForm(FlaskForm):
    ...
    directions = FieldList(
        unbound_field=StringField(label="Directions", validators=[DataRequired()])
    )
    tags = StringField(label="Tags", validators=[DataRequired()])
    notes = FieldList(
        unbound_field=StringField(label="Notes", validators=[DataRequired()])
    )
